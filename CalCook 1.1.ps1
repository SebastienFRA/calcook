...
$config =@(
    ...
    $config | Out-File -FilePath $filePath -Encoding UTF8 -Force

    [System.Windows.MessageBox]::Show("Fichier créé avec succès.", "Information", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
}

if (-Not (Test-Path -Path $filePath)) {
    # Si le fichier n'existe pas, demander à l'utilisateur s'il souhaite le créer
    $resultat = [System.Windows.MessageBox]::Show("Le fichier de configuration est manquant. Voulez-vous le créer pour profiter de CalCook au maximum ?
The configuration file is missing. Do you want to create it to enjoy CalCook to the fullest?", "Fichier Manquant / File missing", [System.Windows.MessageBoxButton]::YesNo, [System.Windows.MessageBoxImage]::Warning)
    
    if ($resultat -eq [System.Windows.MessageBoxResult]::Yes) {
        # Si l'utilisateur choisit "Oui", créer le fichier
        CreerFichierTexte
    } else {
        # Si l'utilisateur choisit "Non", ne rien faire
        [System.Windows.MessageBox]::Show("Aucun fichier n'a été créé.", "Information", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
    }
}else{$global:content = Get-Content $filePath -Encoding UTF8}
...
$global:cout.HorizontalContentAlignment = 'Right'
$global:cout.FontWeight = 'Bold'
$global:cout.Content = ""
$global:cout.Foreground = [Windows.Media.Brushes]::Red
[System.Windows.Controls.Canvas]::SetLeft($global:cout, 680) # Position X
...
$coutpartprix.HorizontalContentAlignment = 'Right'
$coutpartprix.FontWeight = 'Bold'
$coutpartprix.Content = ""
$coutpartprix.Foreground = [Windows.Media.Brushes]::Blue
[System.Windows.Controls.Canvas]::SetLeft($coutpartprix, 680) # Position X
...
$coutlotprix.HorizontalContentAlignment = 'Right'
$coutlotprix.FontWeight = 'Bold'
$coutlotprix.Content = ""
$coutlotprix.Foreground = [Windows.Media.Brushes]::Blue
[System.Windows.Controls.Canvas]::SetLeft($coutlotprix, 680) # Position X
...
$benefprix.HorizontalContentAlignment = 'Right'
$benefprix.FontWeight = 'Bold'
$benefprix.Content = ""
$benefprix.Foreground = [Windows.Media.Brushes]::Green
[System.Windows.Controls.Canvas]::SetLeft($benefprix, 680) # Position X
...
$maj = New-Object System.Windows.Controls.TextBlock
$maj.FontSize = 14
$maj.HorizontalAlignment = 'Center'
$maj.VerticalAlignment = 'Center'
$maj.FontWeight = 'Bold'
$maj.Foreground = [System.Windows.Media.Brushes]::Red
[System.Windows.Controls.Canvas]::SetLeft($maj, 10) # Position X
[System.Windows.Controls.Canvas]::SetTop($maj, 435)  # Position Y

$runText = New-Object System.Windows.Documents.Run
$runText.Text = "Une mise à jour est disponible ($tag) : "

$repo = "SebastienFRA/calcook"
...
if ($tag -gt "1.1"){
$url = "https://github.com/SebastienFRA/calcook/releases/download/$tag/CalCook_"+"$tag"+"_Install.exe"

$hyperlink = New-Object System.Windows.Documents.Hyperlink
$hyperlink.Inlines.Add("ici")
$hyperlink.NavigateUri = "$url"
$hyperlink.Foreground = "Blue"
$hyperlink.ToolTip = "$url"

# Ajouter un gestionnaire pour l'événement Click du lien
$hyperlink.add_Click({
    Start-Process $hyperlink.NavigateUri.AbsoluteUri
})

# Ajouter le texte et le lien au TextBlock
$maj.Inlines.Add($runText)
$maj.Inlines.Add($hyperlink)
}else{
$maj.Visibility = 'Hidden'
}
...
if (-Not (Test-Path -Path $filePath)) {}else
{
...
créer l'ingrédient" }
      ...
      $runText.Text = "La version $tag est disponible : "
...
function langeng {
      ...
      $runText.Text = "Version $tag is available : "
      $hyperlink.Inlines.Clear()
      $hyperlink.Inlines.Add("here")
}
...
Sie die Zutat" }
      ...
      $runText.Text = "EVersion $tag ist verfügbar : "
      $hyperlink.Inlines.Clear()
      $hyperlink.Inlines.Add("hier")
}
...
function langes {
      ...
      $runText.Text = "La versión $tag está disponible : "
      $hyperlink.Inlines.Clear()
      $hyperlink.Inlines.Add("aquí")
}
...
function langit {
      ...
      $runText.Text = "È disponibile la versione $tag : "
      $hyperlink.Inlines.Clear()
      $hyperlink.Inlines.Add("qui")
}
